...
import { useMenuStore } from '../context';
...
const Nav = () => {
	...
	return (
		<div
			...
			className='flex gap-16 px-8 border-b border-slate-200 relative items-center justify-between md:justify-normal'>
			<div className='flex items-center gap-4'>
				...
				<NavLink to={'/'}>
					<h1 className='hidden sm:block text-slate-900 font-bold'>
						Feedyou Bot Tools
					</h1>
				</NavLink>
			</div>
			...
			<List size={30} onClick={toggleMenu} className='md:hidden' />
			{isMenuOpen && (
				<div
					className={`NavItemsMobile fixed bg-white left-0 right-0 flex flex-col gap-2 px-8 py-8 border-b border-slate-200 z-50 md:hidden`}
					...
					{tools.map((tool) => (
						<NavLink
							...
							<span onClick={toggleMenu}>{tool.title}</span>
						</NavLink>
					))}
				</div>
			)}
